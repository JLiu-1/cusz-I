...
constexpr int BLOCK9  = 9;
constexpr int BLOCK32 = 32;
constexpr int BLOCK33 = 33;
...
__global__ void c_spline3d_infprecis_32x16x8data(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    EITER   ectrl,
...
template <
    ...
    typename FP           = float,
    int LINEAR_BLOCK_SIZE = DEFAULT_LINEAR_BLOCK_SIZE>
__global__ void x_spline3d_infprecis_32x16x8data(
    EITER   ectrl,        // input 1
    DIM3    ectrl_size,   //
    STRIDE3 ectrl_leap,   //
...
__forceinline__ __device__ bool xyz33x17x9_predicate(unsigned int x, unsigned int y, unsigned int z,const DIM3 &data_size)
{
    if CONSTEXPR (INCLUSIVE) {  //
            ...
            return (x <= BLOCK32 and y <= BLOCK16 and z <= BLOCK8) and BIX*BLOCK32+x<data_size.x and BIY*BLOCK16+y<data_size.y and BIZ*BLOCK8+z<data_size.z;
    }
    else {
        return x < BLOCK32+(BIX==GDX-1) and y < BLOCK16+(BIY==GDY-1) and z < BLOCK8+(BIZ==GDZ-1) and BIX*BLOCK32+x<data_size.x and BIY*BLOCK16+y<data_size.y and BIZ*BLOCK8+z<data_size.z;
    }
}
...
spline3d_print_block_from_GPU(T volatile a[9][17][33], int radius = 512, bool compress = true, bool print_ectrl = true)
{
    for (auto z = 0; z < ZEND; z++) {
        printf("\nprint from GPU, z=%d\n", z);
        printf("    ");
        for (auto i = 0; i < BLOCK33; i++) printf("%3d", i);
        printf("\n");
...
__device__ void c_reset_scratch_33x17x9data(volatile T1 s_data[9][17][33], volatile T2 s_ectrl[9][17][33], int radius)
{
    ...
    for (auto _tix = TIX; _tix < BLOCK33 * BLOCK17 * BLOCK9; _tix += LINEAR_BLOCK_SIZE) {
        auto x = (_tix % BLOCK33);
        auto y = (_tix / BLOCK33) % BLOCK17;
        auto z = (_tix / BLOCK33) / BLOCK17;
...
{
    auto ax = (TIX % 4) + BIX * 4;//2 is block16 by anchor stride
    ...
    bool interpolator)
{
    ...
    static_assert(COARSEN or BLOCK_DIMX * BLOCK_DIMY * BLOCK_DIMZ <= 384, "block oversized");
    ...
    auto run = [&](auto x, auto y, auto z) {
        ...
        if (xyz33x17x9_predicate<BORDER_INCLUSIVE>(x, y, z,data_size)) {
            ...
            if(BIX == 7 and BIY == 47 and BIZ == 15 and unit==4 and x==4 and y==8 and z==0)
                  ...
                  //  }
            auto global_x=BIX*BLOCK32+x, global_y=BIY*BLOCK16+y, global_z=BIZ*BLOCK8+z;
            /*
    ...
    )
{
    ...
    */
    ...
    if(intp_param.reverse[2]){
        interpolate_stage<
            ...
            false, false, true, LINEAR_BLOCK_SIZE, 4, 3, NO_COARSEN, 2, BORDER_INCLUSIVE, WORKFLOW>(
        ...
        interpolate_stage<
            ...
            false, true, false, LINEAR_BLOCK_SIZE, 9, 2, NO_COARSEN, 2, BORDER_INCLUSIVE, WORKFLOW>(
        ...
        interpolate_stage<
            ...
            true, false, false, LINEAR_BLOCK_SIZE, 9, 5, NO_COARSEN, 1, BORDER_INCLUSIVE, WORKFLOW>(
       ...
       // printf("lv3s0\n");
        interpolate_stage<
            ...
            true, false, false, LINEAR_BLOCK_SIZE, 5, 3, NO_COARSEN, 1, BORDER_INCLUSIVE, WORKFLOW>(
      ...
      //  printf("lv3s2\n");
        interpolate_stage<
            ...
            false, false, true, LINEAR_BLOCK_SIZE, 4, 5, NO_COARSEN, 3, BORDER_INCLUSIVE, WORKFLOW>(
   ...
   // printf("lv3\n");
    ...
    if(intp_param.reverse[1]){
        interpolate_stage<
            T1, T2, FP, decltype(xhollow_reverse), decltype(yhollow_reverse), decltype(zhollow_reverse),  //
            false, false, true, LINEAR_BLOCK_SIZE, 8, 5, NO_COARSEN, 3, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xhollow_reverse, yhollow_reverse, zhollow_reverse, unit, cur_eb_r, cur_ebx2, radius, intp_param.interpolators[0]);
        interpolate_stage<
            T1, T2, FP, decltype(xyellow_reverse), decltype(yyellow_reverse), decltype(zyellow_reverse),  //
            false, true, false, LINEAR_BLOCK_SIZE, 17, 4, NO_COARSEN, 3, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xyellow_reverse, yyellow_reverse, zyellow_reverse, unit, cur_eb_r, cur_ebx2, radius, intp_param.interpolators[1]);
        interpolate_stage<
            T1, T2, FP, decltype(xblue_reverse), decltype(yblue_reverse), decltype(zblue_reverse),  //
            true, false, false, LINEAR_BLOCK_SIZE, 17, 9, NO_COARSEN, 2, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xblue_reverse, yblue_reverse, zblue_reverse, unit, cur_eb_r, cur_ebx2, radius, intp_param.interpolators[2]);
    }
    else{
        interpolate_stage<
            T1, T2, FP, decltype(xblue), decltype(yblue), decltype(zblue),  //
            true, false, false, LINEAR_BLOCK_SIZE, 9, 5, NO_COARSEN, 2, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xblue, yblue, zblue, unit, cur_eb_r, cur_ebx2, radius, intp_param.interpolators[0]);
        interpolate_stage<
            T1, T2, FP, decltype(xyellow), decltype(yyellow), decltype(zyellow),  //
        ...
        interpolate_stage<
            T1, T2, FP, decltype(xhollow), decltype(yhollow), decltype(zhollow),  //
            false, false, true, LINEAR_BLOCK_SIZE, 8, 9, NO_COARSEN, 5, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xhollow, yhollow, zhollow, unit, cur_eb_r, cur_ebx2, radius, intp_param.interpolators[2]);
   ...
   // set_orders(reverse[0]);
    ...
    if(intp_param.reverse[0]){
        ...
        interpolate_stage<
            T1, T2, FP, decltype(xhollow_reverse), decltype(yhollow_reverse), decltype(zhollow_reverse),  //
            false, false, true, LINEAR_BLOCK_SIZE, 16, 9, COARSEN, 5, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xhollow_reverse, yhollow_reverse, zhollow_reverse, unit, cur_eb_r, cur_ebx2, radius, 1);
        interpolate_stage<
            T1, T2, FP, decltype(xyellow_reverse), decltype(yyellow_reverse), decltype(zyellow_reverse),  //
            false, true, false, LINEAR_BLOCK_SIZE, 33, 8, COARSEN, 5, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xyellow_reverse, yyellow_reverse, zyellow_reverse, unit, cur_eb_r, cur_ebx2, radius, 1);
        interpolate_stage<
            T1, T2, FP, decltype(xblue_reverse), decltype(yblue_reverse), decltype(zblue_reverse),  //
            true, false, false, LINEAR_BLOCK_SIZE, 33, 17, COARSEN, 4, BORDER_EXCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xblue_reverse, yblue_reverse, zblue_reverse, unit, cur_eb_r, cur_ebx2, radius, 1);
        ...
        interpolate_stage<
            T1, T2, FP, decltype(xblue), decltype(yblue), decltype(zblue),  //
            true, false, false, LINEAR_BLOCK_SIZE, 17, 9, COARSEN, 4, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xblue, yblue, zblue, unit, cur_eb_r, cur_ebx2, radius, 1);
        interpolate_stage<
            T1, T2, FP, decltype(xyellow), decltype(yyellow), decltype(zyellow),  //
            false, true, false, LINEAR_BLOCK_SIZE, 17, 8, COARSEN, 9, BORDER_INCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xyellow, yyellow, zyellow, unit, cur_eb_r, cur_ebx2, radius, 1);
       
        interpolate_stage<
            T1, T2, FP, decltype(xhollow), decltype(yhollow), decltype(zhollow),  //
            false, false, true, LINEAR_BLOCK_SIZE, 16, 17, COARSEN, 9, BORDER_EXCLUSIVE, WORKFLOW>(
            s_data, s_ectrl,data_size, xhollow, yhollow, zhollow, unit, cur_eb_r, cur_ebx2, radius, 1);
...
__global__ void cusz::c_spline3d_infprecis_32x16x8data(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    EITER   ectrl,
    ...
    )
{
    ...
    /*
       ...
       */
        ...
        c_reset_scratch_33x17x9data<T, T, LINEAR_BLOCK_SIZE>(shmem.data, shmem.ectrl, radius);
        //if(TIX==0 and BIX==0 and BIY==0 and BIZ==0)
        //    printf("reset\n");
        //if(TIX==0 and BIX==0 and BIY==0 and BIZ==0)
        //    printf("dsz: %d %d %d\n",data_size.x,data_size.y,data_size.z);

        global2shmem_33x17x9data<T, T, LINEAR_BLOCK_SIZE>(data, data_size, data_leap, shmem.data);
       ...
       // if(TIX==0 and BIX==0 and BIY==0 and BIZ==0)
        ...
        shmem2global_33x17x9data_with_compaction<T, E, LINEAR_BLOCK_SIZE>(shmem.ectrl, ectrl, ectrl_size, ectrl_leap, radius, compact_val, compact_idx, compact_num);
...
template <
    ...
    typename FP,
    int LINEAR_BLOCK_SIZE>
__global__ void cusz::x_spline3d_infprecis_32x16x8data(
    EITER   ectrl,        // input 1
    DIM3    ectrl_size,   //
    STRIDE3 ectrl_leap,   //
    TITER   anchor,       // input 2
