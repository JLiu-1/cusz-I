...
template <typename E, typename H, typename M>
__global__ void hf_decode_kernel(
    const H* in,             // 压缩数据输入（全局内存）
    const uint8_t* revbook,    // 查找表（全局内存）
    const M* par_nbit,         // 每段压缩数据的总比特数
    const M* par_entry,        // 每段压缩数据在输入数据中的起始偏移
    int revbook_nbyte,         // 查找表字节数
    int sublen,                // 每个线程输出数据的长度（已知预估值）
    int pardeg,                // 总的并行度
    E* out)                    // 解码输出（全局内存）
{
    // 利用共享内存存储查找表
    ...
    if (gid < pardeg) {
        hf_decode_single_thread_inflate_optimized(
            in + par_entry[gid],       // 当前线程对应的数据起始位置
            out + sublen * gid,          // 当前线程的输出区域
